...
export default {
  ...
  setup() {
    ...
    const client = store.getters["client/getClient"];

    async function submitForm(updateClientRequest) {
      loading.value = true;
      ...
      if (response.status === 200) {
        store.dispatch("client/setClient", updateClientRequest);
        router.push("/clients/details/" + updateClientRequest.id);
      } else {
    ...
    return { submitForm, loading, client, goBack };
  },
};
</script>
